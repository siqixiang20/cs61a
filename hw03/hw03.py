...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    if total_weight(end(left_arm))*length(left_arm) != total_weight(end(right_arm))*length(right_arm):
        return False
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        return tree(total_weight(m), branches=[totals_tree(left_mobile),totals_tree(right_mobile)])
...
def sub_interval(x, y):
    ...
    and any value in y."""
    z0 = lower_bound(x) - upper_bound(y)
    z1 = upper_bound(x) - lower_bound(y)
...
def quadratic(x, a, b, c):
    ...
    if extreme_point > lower_bound(x) and extreme_point < upper_bound(x):
        return interval(min(left_value,right_value, extreme_value),max(left_value,right_value, extreme_value))
    else:
