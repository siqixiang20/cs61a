...
def subseqs(s):
    ...
    if s == []:
        ...
        old_seq = subseqs(s[1:])
        return old_seq + insert_into_all(s[0], old_seq)
...
def test(g):  ### Another solution for make_generators_generator
    def gen(i):
        f = g()
        for x in range(i):
            yield next(f)          
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        ...
        person_count = 0
        def person_counter(input_info):
            nonlocal person_count, global_count
            if input_info == 'count':
                person_count += 1
                return person_count
            elif input_info == 'reset':
                person_count = 0
            elif input_info == 'global-count':
                global_count += 1
                return global_count
            elif input_info == 'global-reset':
                global_count = 0
