...
def every_other(s):
    ...
    Link(4)
    """
    if s and s.rest is not Link.empty:
        s.rest = s.rest.rest
        every_other(s.rest)
...
def has_cycle_constant(link):
    ...
    if link is Link.empty:
        return False
    link1, link2 = link, link.rest
    while link2 is not Link.empty:
        if link2.rest is Link.empty:
            return False
        if link1 == link2 or link1 == link2.rest:
            return True
        link1, link2 = link1.rest, link2.rest.rest
    return False
