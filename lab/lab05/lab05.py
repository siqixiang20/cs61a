def couple(s, t):
    ...
    """
    assert len(s) == len(t)
    couple_list = [[] for i in range(len(s))]
    for i in range(len(s)):
        couple_list[i] = [s[i],t[i]]
    return couple_list
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(city_a,point)<=distance(city_b,point) else get_name(city_b)
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        return sum(berry_finder(x) for x in branches(t)) > 0
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        bs = [sprout_leaves(x, leaves) for x in branches(t)]
...
def add_trees(t1, t2):
    ...
    if len_1 == len_2:
        return tree(label(t1)+label(t2), [add_trees(x1,x2) for x1, x2 in zip(branches(t1),branches(t2))])
    elif len_1 < len_2:
        new_t1_branches = branches(t1)+[tree(0) for i in range(len_2-len_1)]
        new_t1 = tree(label(t1), branches= new_t1_branches)
