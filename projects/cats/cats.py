...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def contain(s):
        para_list = split(remove_punctuation(lower(s)))
        m = 0
        for x in para_list:
            if x in topic:
                m = 1
...
def wpm(typed, elapsed):
    ...
    if len(typed) == 0:
        ...
        wpm = (len(typed)/5.0)*(60.0/elapsed)
        return wpm
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        d_func = lambda x: diff_function(user_word, x, limit)
        return min(valid_words, key = d_func) if min([d_func(x) for x in valid_words])<= limit else user_word      
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    for times in times_per_player:
        ...
        times_per_word += [interval]
    results = game(words, times_per_word)
    return results
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for i in word_indices:
        for j in player_indices:
            if time(game, j, i) <= min([time(game, player, i) for player in player_indices]):
                word_list[j] += [word_at(game, i)]
                break
    return word_list 
    # END PROBLEM 10
